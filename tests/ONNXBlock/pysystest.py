...
class PySysTest(AnalyticsBuilderBaseTest):
	...
	def execute(self):
		self.correlator = self.startAnalyticsBuilderCorrelator(blockSourceDir=f'{self.project.SOURCE}/blocks/', arguments=["--config", f"{self.project.SOURCE}/blocks/ONNX/plugin.yaml", "--config", f"{self.project.SOURCE}/blocks/Python/plugin.yaml"])
		
		self.modelId = self.createTestModel('apamax.analyticsbuilder.samples.ONNX', {
		...
		self.sendEventStrings(self.correlator,
								...
								self.timestamp(2.1),
								self.inputEvent('value2', 17.25, id=self.modelId, properties={'value2': 'value'}),
								self.timestamp(2.5),
							  )
	...
	def validate(self):
		...
		self.assertBlockOutput('result1', [20, 29.5])
